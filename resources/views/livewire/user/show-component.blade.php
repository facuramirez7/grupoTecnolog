            ...
            <div class="relative z-0 w-full mb-6 group">
                ...
                <select wire:model='rol_id'
                    ...
                    @foreach ($roles as $rol)
                        <option value="{{ $rol->id }}" wire:key="rol-{{ $rol->id }}" >{{ $rol->name }}</option>
                    @endforeach
                </select>
            </div>
      

            <x-primary-button id="myButton" type="submit" wire:click.prevent="edit({{ $user->id }})" wire:loading.attr="disabled" disabled >
                Editar usuario <i class="fa-solid fa-pencil"></i>
            </x-primary-button>
    </form>
    <script>
        ...
        myInput2.addEventListener("input", enableButton);
    </script>
</div>
