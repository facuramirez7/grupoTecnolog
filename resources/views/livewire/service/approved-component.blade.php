<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <button x-data x-on:click="$dispatch('open-modal',{name:'create'})"
        class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm m-4 px-3 py-1 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Añadir servicio <i class="fa-solid fa-plus"></i></button>
        ...
        <tbody>
            @foreach ($services as $service)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4">
                        <a href="/servicio/{{ $service->id }}">
                            <x-info-button>
                                <i class="fa-solid fa-info pl-1 pr-1"></i>
                            </x-info-button></a>

                        <x-danger-button wire:click="destroyService( {{ $service->id }})" wire:loading.attr="disabled">
    ...
    <x-modal-model title="Crear servicio" name="create">
        @slot('body')
            <form class="p-4">
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <input wire:model='prize' type="number" step="0.01" id="prize"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " required />
                ...
                @slot('button')
                    <x-green-button type="submit" wire:click.prevent="createService" wire:loading.attr="disabled">
